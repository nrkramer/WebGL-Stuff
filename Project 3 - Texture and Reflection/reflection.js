...
// Skybox
var aSkyPosition; // vertex position
var uSkyProjection; // projection matrix
var uSkyView; // view matrix

// Light
var lightDir = [1.0, -1.0, -1.0];
var lightMatrix = lookAt([0.0, 3.0, 0.0], lightDir, [0.0, 1.0, 0.0]);
...
// Attributes
var aPosition; // vertex positions
var aVertexColor; // vertex colors
var aTextureCoord; // texture coordinates
var aNormal; // normals
var aPositionShadow; // shadow map coordinates
...
var uReflective; // reflective boolean
...
window.onload = function init(){
	...
	shadowMatrix = ortho(-20, 20, -20, 20, -20, 20);
	...
	aSkyPosition = gl.getAttribLocation(skyboxProgram, "aPosition");
	uSkyProjection = gl.getUniformLocation(skyboxProgram, "projection");
	uSkyView = gl.getUniformLocation(skyboxProgram, "view");
	gl.useProgram(skyboxProgram);
	gl.uniform1i(gl.getUniformLocation(skyboxProgram, "uSkyCubeSampler"), 0);
	gl.uniformMatrix4fv(uSkyProjection, false, flatten(projectionMatrix)); // set projection matrix
	skybox.bufferData(); // set up cube map

	// now set-up using our main shader program (model renderer)
	...
	gl.uniform1i(gl.getUniformLocation(program, "skyboxSampler"), 2); // skybox sampler
...
function updateCamera() {
	...
	skyViewMatrix = mult(rotationMat, lookAt(vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 1.0), vec3(0.0, 1.0, 0.0)));
...
function renderSkybox() {
	gl.depthMask(false);
	gl.useProgram(skyboxProgram);
	gl.enableVertexAttribArray(aSkyPosition);

	gl.uniformMatrix4fv(uSkyView, false, flatten(skyViewMatrix)); // assign view matrix
	skybox.drawSkyBox();

	gl.disableVertexAttribArray(aSkyPosition);
	gl.depthMask(true);
}
...
function render() {
	...
	gl.uniformMatrix4fv(viewPtr, 0, flatten(viewMatrix)); // set view matrix
	gl.uniformMatrix4fv(PMatrixLight, false, flatten(shadowMatrix)); // set shadow matrix
	gl.uniformMatrix4fv(LMatrixLight, false, flatten(lightMatrix)); // set light matrix

	for(var i = 0; i < models.length; i++) {
		// set reflective boolean
		if (models[i].reflective) {
			gl.uniform1i(uReflective, true);
			gl.activeTexture(gl.TEXTURE2);
			gl.bindTexture(gl.TEXTURE_CUBE_MAP, skybox.cubeMap);
		} else {
			gl.uniform1i(uReflective, false);
		}
